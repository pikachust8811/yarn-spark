...
RUN curl -s http://apache.stu.edu.tw/hadoop/common/hadoop-2.6.5/hadoop-2.6.5.tar.gz | \
    tar -xz -C /usr/local/ && \
...
RUN curl -s https://archive.apache.org/dist/spark/spark-1.6.1/spark-1.6.1-bin-hadoop2.6.tgz | \
    tar -xz -C /usr/local/ && \
